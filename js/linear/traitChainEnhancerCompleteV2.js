...
import { getSlotsFromFormation, movePlayer } from "../lineup.js?v=strict-cdm-1";
...
function highlightCompatibleSlots(selectedPlayerId) {
  ...
  slots.forEach(slot => {
    const previewTeam = createPreviewTeam(team);
    if (!movePlayer(previewTeam, selectedPlayerId, slot.key)) return;
...
function createPreviewTeam(team) {
  return {
    ...team,
    lineup: { ...(team.lineup || {}) },
