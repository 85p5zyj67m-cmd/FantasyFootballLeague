...
function simulateBackgroundMatches(season, matches) {
  matches.forEach(m => registerMatchResult(season, { ...m, ...simulateMatchWithEvents(m.home, m.away, { knockout: m.type === "KNOCKOUT" }), userMatch: false }, false));
}
...
function simulateMatchWithEvents(home, away, options = {}) {
  ...
  events.push(...generateCardEvents(home, away));
...
function playMinutes(c) {
  ...
  for (let minute = c.from; minute <= c.to; minute++) {
    ...
    if (Math.random() < goalChance) { if (isHome) goals.home++; else goals.away++; const scorer = pickScorer(team, type); const assister = Math.random() < 0.62 ? pickAssister(team, scorer) : null; c.events.push({ minute, type: "GOAL", text: goalText(minute, team, scorer, type, attacker, assister) }); }
...
function goalText(minute, team, scorer, type, profile, assister) { const s = profile.chainLabels.length && Math.random() < 0.5 ? " The active chain makes the difference." : ""; const a = assister ? ` Assist: ${assister}.` : ""; if (type === "cross") return `${minute}' GOAL! ${team.name} finish a dangerous cross through ${scorer}.${a}${s}`; if (type === "through") return `${minute}' GOAL! ${team.name} break the line with a through ball and ${scorer} scores.${a}${s}`; if (type === "cutback") return `${minute}' GOAL! ${team.name} create a cutback and ${scorer} converts.${a}${s}`; if (type === "setPiece") return `${minute}' GOAL! ${team.name} punish the set piece. ${scorer} gets the final touch.${a}${s}`; if (type === "longShot") return `${minute}' GOAL! ${scorer} fires in from distance for ${team.name}.${s}`; if (type === "transition") return `${minute}' GOAL! ${team.name} explode in transition and ${scorer} finishes.${a}${s}`; if (type === "magic") return `${minute}' GOAL! A moment of genius from ${team.name}, finished by ${scorer}.${a}${s}`; return `${minute}' GOAL! ${team.name} strike through ${scorer}.${a}${s}`; }
...
function pickAssister(team, scorerName) { const pool = getStarters(team).filter(p => p.position !== "GK" && p.name !== scorerName); if (!pool.length) return null; const weighted = pool.flatMap(p => { const traits = getTraitList(p).map(x => String(x).toLowerCase()); let w = p.position === "MID" ? 4 : p.position === "ATT" ? 2 : 1; if (traits.includes("vision playmaker") || traits.includes("deep playmaker") || traits.includes("advanced playmaker") || traits.includes("connector") || traits.includes("creative genius")) w += 3; return Array.from({ length: Math.max(1, Math.round(w)) }, () => p); }); const p = weighted[Math.floor(Math.random() * weighted.length)]; return p ? p.name : null; }
function pickCardedPlayer(team) { const pool = getStarters(team); if (!pool.length) return null; const weighted = pool.flatMap(p => { const w = p.position === "DEF" ? 3 : p.position === "MID" ? 2 : 1; return Array.from({ length: w }, () => p); }); return weighted[Math.floor(Math.random() * weighted.length)] || null; }
function generateCardEvents(home, away) { const events = []; [home, away].forEach(team => { const roll = Math.random(); const count = roll < 0.45 ? 1 : roll < 0.72 ? 2 : roll < 0.85 ? 3 : 0; for (let i = 0; i < count; i++) { const player = pickCardedPlayer(team); if (!player) continue; const minute = Math.floor(randomBetween(8, 89)); const isRed = Math.random() < 0.05; events.push({ minute, type: "CARD", text: isRed ? `${minute}' Red card for ${player.name} (${team.name}).` : `${minute}' Yellow card for ${player.name} (${team.name}).` }); } }); return events; }
